...
import frc.team1523.robot.commands.DriveJoystick;
...
public class Drive extends Subsystem {
	...
	public void drive(Joystick stick) {
		drive.mecanumDrive_Cartesian(joystickCorrection(stick.getX(), 0.1), joystickCorrection(stick.getY(), 0.1),
				-joystickCorrection(stick.getZ(), 0.198), 0);
	}
	...
	private double deadband(double input, double value) {
		if (Math.abs(input) < value)
			...
			return (input - value) / (1 - value); //Positive return

		else
			return -(Math.abs(input) - value) / (1 - value); //Negative return
	}
